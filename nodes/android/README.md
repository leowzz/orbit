...
4. 点击卡片右上角添加到桌面；应用会区分桌面不支持一键添加与请求被拒绝，前者无需打开 Orbit 权限页。栖桌面可长按空白处进入编辑 → 左下角「添加小部件」→「应用小部件」→ Orbit。不支持固定组件的启动器可长按桌面 → 小组件 → Orbit，分别添加「用量」「Session 状态」。点击桌面组件返回应用。小米 HyperOS 路径为：长按桌面 → 小部件 → 全部应用 → 安卓小部件 → Orbit。

低高度横屏（逻辑宽度 ≥600、高度 <500）使用顶部「状态 / 配置」切换；用量和 Session 卡片并排，配置页底部固定测试、二维码和保存按钮。扫码配置使用全屏横向布局，二维码按可用高度缩放，关闭按钮始终可见。竖屏保留纵向页面。
