...
交互终端顶部固定显示前端地址和进程状态，下方按 `[Core]` / `[Vite]` 显示滚动日志。
Ctrl-C 停止两个服务及其子进程；任一服务退出时也会停止另一个服务。Go 代码修改仍需重启。
日志重定向、`TERM=dumb`、`ORBIT_DEV_PLAIN=1 make dev-core` 或同时启动 Agent 的 `make dev` 使用普通日志输出。`7620` 是后端及嵌入页面入口，
